" USAGE:
" Place your ~/.vim/.jshint.js file with options. Must be a valid javascript comment.
"
...
"jordwalke: au BufLeave <buffer> call s:JSLintClear()
"jordwalke: au BufEnter <buffer> call s:JSLint()
"jordwalke: au InsertLeave <buffer> call s:JSLint()
"jordwalke: au InsertEnter <buffer> call s:JSLint()
"jordwalke:
  au InsertLeave <buffer> call s:JSLintClear()
"jordwalke:
  au InsertEnter <buffer> call s:JSLintClear()
...
"jordwalke: TODO: Don't comment these out, rather update the error maps offsets!
"and remove any errors on that particular line.
"jordwalke: noremap <buffer><silent> dd dd:JSLintUpdate<CR>
"jordwalke: noremap <buffer><silent> dw dw:JSLintUpdate<CR>
"jordwalke: noremap <buffer><silent> u u:JSLintUpdate<CR>
"jordwalke: noremap <buffer><silent> <C-R> <C-R>:JSLintUpdate<CR>
"jordwalke:
  noremap <buffer><silent> u u:JSLintClear<CR>
"jordwalke:
  noremap <buffer><silent> <C-R> <C-R>:JSLintClear<CR>

" Set up command and parameters
if has("win32")
  let s:jsCommand = 'cscript /NoLogo '
  let s:runjslint_ext = 'wsf'
else
  let s:runjslint_ext = 'js'
  if exists("g:jsCommand")
    let s:jsCommand = g:jsCommand
  elseif executable('/System/Library/Frameworks/JavaScriptCore.framework/Resources/jsc')
    let s:jsCommand = '/System/Library/Frameworks/JavaScriptCore.framework/Resources/jsc'
  elseif executable('node')
    let s:jsCommand = 'node'
...
" Maybe the following cd only should be done in *NIX envs.
let s:cmd = "cd " . s:plugin_path . " && " . s:jsCommand . " " . s:plugin_path . "runjslint." . s:runjslint_ext
let s:transformCmd = s:jsCommand ." " . s:plugin_path . "/JSXTransformer/runner.js"

let s:jslintrc_file = expand('~/.vim/.jshint.js')
...
function! s:JSLint()
  ...
  let oneLintLintRc = join(s:jslintrc, "")
  ...
  if v:shell_error
    ...
    " as lint error.
    let syntaxError = matchstr(substitute(processed, "\n", " ", "g") , "Error: Line [0-9]*:")

    " Match {line}:{char}:{message}
    " SIMULATES A JS LINT ERROR!
    let lineNumOfError = matchstr(syntaxError, '[0-9]\+')
    let errors = lineNumOfError . ':' . '0' . ':WARNING:JSX cannot transform file due to syntax error\n'
  else
    let errors = system(s:cmd, processed)
    if v:shell_error
  ...
  for error in split(errors, "\n")
    ...
    if !empty(b:parts)
      let l:line = b:parts[1] + (b:firstline - 1 ) " Get line relative to selection - subtract off lintrc prefix
      let l:third = b:parts[4]
      let l:fourth = b:parts[4]
      let l:errorMessage = empty(b:parts[4]) ? b:parts[3] : b:parts[4]

      " For some reason, on win32, the ERROR field is missing as the third
      " entry, we detect if it is missing from the middle and ensure to
      " interpret the third entry as the message (in that case).
      " TODO: Test this on mac os x and find out why win32 ommmits ERROR
      if empty(b:parts[4])
        let l:errorType = 'E'
      else
        if b:parts[3] == 'ERROR'
